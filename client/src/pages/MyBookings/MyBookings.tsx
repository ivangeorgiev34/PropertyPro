...
export const MyBookings: React.FC = () => {
  ...
  const onSeacrhSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await getUsersBookingsBySearch(
        token!,
        searchOption,
        searchValue,
        1
  ...
  const onNextPageClick = async () => {
    ...
    if (searchValue === "") {
      ...
      setSearchParams(
        `${searchOption.toLowerCase()}=${searchValue}&page=${page + 1}`
      );
    }
  };
  ...
  return (
    <div className={styles.bookingCardsWrapper}>
      ...
      <div className={styles.viewAllBtnErrorsContainer}>
        <button className={styles.viewAllBtn} onClick={onViewAllBtnClick}>
          View All
        </button>
      ...
      <div className={styles.bookingCardContainer}>
        {myBookings.map((b: IMyBookings) => {
          return (
            <Booking key={b.id} {...b} onBookingDelete={onBookingDelete} />
          );
        })}
      </div>
