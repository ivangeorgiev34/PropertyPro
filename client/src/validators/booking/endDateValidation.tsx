export const endDateValidation = (startDate: string, endDate: string): string => {
    if (Number.isNaN(Date.parse(endDate))) {
        return "Invalid date, fill the fields that are missing"
    }

    if (Date.parse("2025-01-01") <= Date.parse(endDate)) {
        return "Date must be before 1st of January 2025";
    } else if (Date.parse("2023-09-01") >= Date.parse(endDate)) {
