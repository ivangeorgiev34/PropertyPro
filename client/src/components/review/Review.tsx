...
export const Review: React.FC<IReviewProps> = ({
  ...
  const generateStarReviews = (): React.ReactNode => {
    const yellowStarsArray = Array.from<HTMLElement>({ length: stars }).map(
      (el, index) => {
        return (
          <i
            key={`yellow-${index}`}
            className={`fa-solid fa-star ${styles.yellowStar}`}
    ...
    const blackStarsArray = Array.from<HTMLElement>({
      length: 5 - yellowStarsArray.length,
    }).map((el, index) => {
      return (
        <i
          key={`black-${index}`}
          className="fa-solid fa-star"
          data-testid="black-star"
        ></i>
      );
    });
  ...
  const onDeleteReviewClick = async () => {
    ...
    if (
      window.confirm("Are you sure you want to delete your review?") === true
    ) {
    ...
    <div className={styles.card}>
      <div className={styles.tenantInfoContainerWrapper}>
        <div className={styles.tenantInfoContainer}>
          <img
            className={styles.tenantProfilePicture}
            src={
          ...
          <div className={styles.reviewStarsContainer}>
            <h3 className={styles.tenantNames}>
              {tenant.firstName} {tenant.lastName}
            </h3>
            <span className={styles.author}>
              {tenant.id === userId ? "By Me" : null}
            </span>
            <div>{generateStarReviews()}</div>
          </div>
        </div>
        <hr />
      ...
      {tenant.id === userId ? (
        <div className={styles.reviewBtnsContainerWrapper}>
          <div className={styles.reviewBtnsContainer}>
            <Link className={styles.reviewEditBtn} to={`/review/edit/${id}`}>
              Edit
            </Link>
            <button
          ...
          <span data-testid="delete-error" className={styles.error}>
            {deleteError}
          </span>
        </div>
      ) : null}
