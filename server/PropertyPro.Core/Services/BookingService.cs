...
namespace PropertyPro.Core.Services
{
	public class BookingService : IBookingService
	{
			...
			var bookingDto = await repo.All<Booking>()
				...
				.Select(b => new BookingDto()
				{
					...
					Property = new PropertyDto()
					{
						...
						GuestPricePerNight = b.Property.GuestPricePerNight,
		...
		public async Task<BookingDto> EditBookingAsync(EditBookingDto editBookingDto, string bookingId, string userId, DateTime startDate, DateTime endDate)
		{
			...
			var bookingDto = new BookingDto()
			{
				...
				Property = new PropertyDto()
				{
					...
					GuestPricePerNight = booking.Property.GuestPricePerNight,
		...
		public async Task<List<BookingDto>?> GetAllUsersBookingsBySearchAsync(GetAllUsersBookingsSearchParameters searchParameters, string userId)
		{
			var bookings = await repo.All<Booking>()
				.Include(b => b.Property)
				.ThenInclude(p => p.Landlord)
				.ThenInclude(l => l.User)
				.Include(b => b.Tenant)
			...
			if (searchParameters.Title != null)
			{
				searchedBookings = bookings
					.AsQueryable()
					.Where(b => b.Property.Title.Contains(searchParameters.Title, StringComparison.OrdinalIgnoreCase))
					.ToList();
			}
		...
		public async Task<BookingDto?> GetBookingByIdAsync(string bookingId, string userId)
		{
			var booking = await repo.All<Booking>()
			   .Include(b => b.Property)
				...
				.Include(b => b.Tenant)
				.ThenInclude(t => t.User)
				.Where(b => b.IsActive == true && b.Id == Guid.Parse(bookingId) && b.Tenant.UserId == Guid.Parse(userId))
				.Select(b => new BookingDto()
				{
					Id = b.Id,
					StartDate = b.StartDate,
